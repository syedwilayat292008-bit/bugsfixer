...
const Inventory = () => {
  // --- Enterprise SEO ---
  useEffect(() => {
    document.title = "Hardware Vault: Trusted Brands & Clients | BugsFixer Pakistan";
    ...
    setMetaTag('name', 'description', 'BugsFixer Hardware Vault - Trusted vendor brands (Hikvision, Cisco, ZKTeco, Dell) and corporate clients including FBR, Supreme Court, State Life, and Wafid Visa Centers.');
    setMetaTag('name', 'keywords', 'Hardware Vault, Trusted IT Brands Pakistan, Hikvision Partner, Cisco Networking, Enterprise IT Vendors, FBR IT Partner');
  }, []);
    ...
    <div className="min-h-screen bg-slate-50/70 pt-28 pb-24 relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-4 lg:px-8 relative z-10">
        ...
        <motion.div 
          ...
          className="mb-16 bg-white rounded-[2.5rem] p-8 lg:p-12 shadow-xl shadow-slate-200/50 border border-slate-100 relative"
        >
          <div className="text-center max-w-2xl mx-auto mb-10">
            <span className="text-xs font-black uppercase tracking-widest text-blue-600 block mb-2">Institutional Clients</span>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-3">Trusted By Leading Organizations</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {trustedClients.map((client, idx) => (
              <motion.div 
                ...
                className="flex flex-col items-center text-center gap-3 p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-blue-200 hover:bg-white hover:shadow-md transition-all"
              >
                <div className={`p-3 bg-white rounded-xl shadow-sm ${client.color}`}>
                  {client.icon}
                </div>
                <div>
                  <p className="font-black text-slate-900 text-sm tracking-tight leading-tight">{client.name}</p>
        ...
        >
          <div className="text-center max-w-3xl mx-auto mb-12">
            <span className="text-xs font-black uppercase tracking-widest text-indigo-600 block mb-2">Authorized Brand Portfolio</span>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-3">Tier-1 Vendor Partnerships</h2>
            <p className="text-sm text-slate-500 font-medium">
            ...
            {brandCategories.map((cat, idx) => (
              ...
              >
                <div className="flex items-center gap-4 mb-6 pb-4 border-b border-slate-100">
                  ...
                  <h3 className="text-lg font-black text-slate-900 leading-tight">{cat.title}</h3>
                </div>
                
                <div className="space-y-3">
                  {cat.brands.map((brand, bIdx) => (
                    <div key={bIdx} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 hover:border-blue-100 transition-colors">
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
          className="mb-16 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white rounded-[2.5rem] p-8 lg:p-12 border border-slate-700 shadow-2xl relative overflow-hidden"
        >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 shrink-0">
              ...
              <div className="px-5 py-3 rounded-2xl bg-slate-800/80 border border-slate-700 text-center">
                ...
                <p className="text-xs text-slate-300 font-bold mt-0.5">Vendor Backed</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
          className="text-center bg-white rounded-[2.5rem] p-10 lg:p-14 shadow-xl shadow-slate-200/50 border border-slate-100"
        >
          ...
          <p className="text-slate-500 font-medium max-w-2xl mx-auto mb-8 leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              to="/build-bundle"
              className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-wider flex items-center gap-2 shadow-xl shadow-blue-900/20 transition-all"
            >
              Build IT Bundle <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              to="/contact"
              className="bg-slate-100 hover:bg-slate-200 text-slate-900 px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-wider flex items-center gap-2 border border-slate-200 transition-all"
            >
